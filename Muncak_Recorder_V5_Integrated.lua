...
local function v5AdaptiveInterval(sample)
    ...
    if speed <= V5_CONFIG.idleSpeed then
        return V5_CONFIG.idleInterval
    elseif speed >= V5_CONFIG.fastSpeed then
        return V5_CONFIG.fastInterval
    end

    local alpha = (speed - V5_CONFIG.idleSpeed) /
        (V5_CONFIG.fastSpeed - V5_CONFIG.idleSpeed)
    return V5_CONFIG.idleInterval +
        (V5_CONFIG.fastInterval - V5_CONFIG.idleInterval) * v5Clamp(alpha, 0, 1)
end
...
StartRecord.MouseButton1Click:Connect(function()
    ...
    task.spawn(function()
        ...
        while State.Recording do
            ...
            if shouldRecord then
                ...
                if lastRetained then
                    ...
                    if prevCF then
                        ...
                        local dv = (sample.velocity - (lastRetained.velocity or Vector3.zero)).Magnitude

                        keep =
                            d >= V5_CONFIG.keepDistance
                            or rotation >= V5_CONFIG.keepRotation
                            or dv >= V5_CONFIG.keepVelocityDelta
                            or sample.acceleration.Magnitude >= V5_CONFIG.keepAccelerationDelta
                            or d >= V5_CONFIG.teleportDistance
                    end
                end
            ...
            task.wait(math.clamp(nextSampleInterval * 0.5, 1/120, 1/20))
        end
...
local function getSmoothCFrame(path, index, alpha)
    ...
    local v0 = a.velocity or v5Velocity(a, b)
    local v1 = b.velocity or v5Velocity(a, b)
...
local function playPath(path)
    ...
    -- Normalize JSON-friendly V5 vectors after loading.
    ...
    task.spawn(function()
        ...
        while State.Playing do
            ...
            -- Smoothstep is retained only as a light time-domain easing.
            ...
            if root then
                ...
                if cfa and cfb then
                    ...
                    if distance >= V5_CONFIG.teleportDistance
                        or duration >= V5_CONFIG.maxInterpolationGap then
